...
export const FacultyAnalytics: React.FC = () => {
  ...
  const backlogSource = analytics?.backlogAnalysis ?? {};
  const topPerformers = Array.isArray(analytics?.topPerformers)
    ? analytics.topPerformers
    : [];
  const atRiskStudents = Array.isArray(analytics?.atRiskStudents)
    ? analytics.atRiskStudents
    : [];

  const cgpaChartData = Object.keys(cgpaSource).map((key) => ({
    range: key,
    count: Number(cgpaSource[key] ?? 0),
  }));

  const attendanceChartData = Object.keys(attendanceSource).map((key) => ({
    range: key,
    count: Number(attendanceSource[key] ?? 0),
  }));

  const backlogChartData = Object.keys(backlogSource).map((key) => ({
    name: key,
    value: Number(backlogSource[key] ?? 0),
  }));
  ...
  const handleExportCSV = () => {
    ...
    const rows = topPerformers.map((p: any, idx: number) => [
      idx + 1,
      p.hallTicketNumber,
      p.name,
      p.cgpa,
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Analyze performance distributions, backlog analysis, and identify
            at-risk students.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          ...
          <p className="text-2xl font-bold text-slate-850 dark:text-slate-200 mt-1">
            {analytics.totalAssignedStudents}
          </p>
        </div>
        <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          <p className="text-[10px] text-slate-400 font-bold uppercase">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
            <TrendingUp className="w-4 h-4 text-emerald-500" /> Class Top
            Performers
          </h3>
          <Table
