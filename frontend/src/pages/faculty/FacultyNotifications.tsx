...
export const FacultyNotifications: React.FC = () => {
  ...
  const { data: notificationsRes, isLoading } = useQuery({
    ...
    select: (res) => {
      ...
      return res.data ? res.data.filter((n: any) => n.senderId === user?.id) : [];
    },
  });
  ...
  const sectionOptions = [...new Set(assignments.map((a: any) => a.section))].map((sec: any) => ({
    label: `Section ${sec}`,
    value: sec,
  }));
  ...
  return (
    <div className="space-y-6 text-left">
      <div>
        <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">Class Announcements</h2>
        <p className="text-xs text-slate-400">Post announcements and notification alerts to your assigned sections.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm h-fit">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" id="announcement-form">
            <Select
              {...register('type')}
              options={typeOptions}
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Post announcement history log</h3>
          <Table
            columns={columns}
            data={notifications}
            isLoading={isLoading}
