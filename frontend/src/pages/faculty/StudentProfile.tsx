...
import { ArrowLeft, GraduationCap, BookOpen, Clock, Activity } from 'lucide-react';
...
export const StudentProfile: React.FC = () => {
  ...
  if (isError || !response?.data) {
    return (
      <div className="text-center py-12 space-y-4">
        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">Error Loading Student</h3>
        <p className="text-sm text-slate-500">You may not be authorized to view this student profile or student doesn't exist.</p>
  ...
  const tabOptions = [
    ...
    { id: 'timeline', label: 'Timeline History', icon: <Activity className="w-4 h-4" /> },
  ];
  ...
  return (
    <div className="space-y-6 text-left">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate('/faculty/students')} id="btn-back-roster">
          <ArrowLeft className="w-4 h-4 mr-2" /> Back to Roster
        </Button>
      </div>

      <StudentProfileCard student={profile.student} />
        ...
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="text-xs space-y-2 text-slate-600 dark:text-slate-400">
              <p><strong>Father Name:</strong> {profile.student.fatherName || 'N/A'}</p>
              <p><strong>Mother Name:</strong> {profile.student.motherName || 'N/A'}</p>
              <p><strong>Mobile Number:</strong> {profile.student.mobile || 'N/A'}</p>
              <p><strong>Official Email:</strong> {profile.student.email || 'N/A'}</p>
            </div>
          </div>
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Identity Details</h3>
            <div className="text-xs space-y-2 text-slate-600 dark:text-slate-400">
              <p><strong>Aadhaar Number:</strong> {profile.student.aadhaarNumber || 'N/A'}</p>
              <p><strong>ABC ID:</strong> {profile.student.abcId || 'N/A'}</p>
              <p><strong>Gender:</strong> {profile.student.gender || 'N/A'}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'attendance' && (
        <div className="space-y-6">
          ...
          <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm flex items-center gap-6">
            <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-850 flex items-center justify-center shrink-0 border border-slate-200/50 dark:border-slate-800">
              <span className={`text-lg font-bold ${
                profile.attendanceSummary.percentage >= 75 ? 'text-emerald-500' : 'text-rose-500'
              }`}>
                {profile.attendanceSummary.percentage}%
              </span>
            </div>
            <div>
              <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Overall Attendance</h4>
              <p className="text-xs text-slate-450 mt-1">
                Present for {profile.attendanceSummary.present} class(es) out of {profile.attendanceSummary.total} total sessions.
              </p>
            </div>
          </div>
          ...
          <Table
            ...
            data={profile.attendanceSummary?.subjectWise || []}
            isLoading={false}
      ...
      {activeTab === 'timeline' && (
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-850 dark:text-slate-200">Academic & Activity Timeline</h3>
          ...
          <div className="relative border-l border-slate-200 dark:border-slate-800 ml-4 pl-6 space-y-6">
            {!profile.timeline || profile.timeline.length === 0 ? (
              <p className="text-xs text-slate-500 py-4 pl-2">No timeline activities recorded yet.</p>
            ) : (
              profile.timeline.map((event: any, idx: number) => (
                <div key={idx} className="relative">
                  {/* Timeline dot */}
                  <div className={`absolute -left-[31px] top-1 w-3 h-3 rounded-full border-2 border-white dark:border-slate-900 ${
                    event.type === 'admission' ? 'bg-sky-500' :
                    event.type === 'academic' ? 'bg-emerald-500' :
                    event.type === 'certificate' ? 'bg-purple-500' :
                    'bg-amber-500'
                  }`} />
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{event.title}</span>
                      <span className="text-[9px] text-slate-500 font-mono">{new Date(event.date).toLocaleDateString()}</span>
                    </div>
