      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            View circular notifications, holiday alerts, and announcements from
            HOD/faculty.
          </p>
        </div>
        {unreads.length > 0 && (
        ...
        {list.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-slate-250 dark:border-slate-800 rounded-2xl">
            <p className="text-sm text-slate-400">
              No circular notifications available.
            </p>
          </div>
        ) : (
          list.map((notif: any) => {
                ...
                <div className="flex-grow space-y-2">
                  <div className="flex justify-between items-center gap-4">
                    <div className="flex items-center gap-2">
                      <h4
                        className={`text-sm font-semibold text-slate-850 dark:text-slate-200 ${!isRead ? "font-bold" : ""}`}
                      >
                        {notif.title}
                      </h4>
                      {!isRead && (
