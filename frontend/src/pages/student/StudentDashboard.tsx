    ...
    <div className="space-y-6 text-left">
      ...
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl text-white relative overflow-hidden">
        ...
        <p className="text-xs text-slate-400 mt-1">
          Welcome to your Campus360 Student portal. HT Number: {stats.hallTicketNumber}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm text-left space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
              <Bell className="w-4.5 h-4.5 text-primary-500 animate-bounce" /> Recent Announcements
            </h3>
            <Link
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-850">
            {stats.recentNotifications.length === 0 ? (
              <p className="text-xs text-center py-6 text-slate-400">No recent announcements posted.</p>
            ) : (
              stats.recentNotifications.map((notif: any) => (
                <div key={notif._id} className="py-3.5 first:pt-0 last:pb-0">
                  <div className="flex justify-between items-start gap-2">
                    <h4 className="text-xs font-semibold text-slate-850 dark:text-slate-200">{notif.title}</h4>
                  ...
                  <p className="text-[10px] text-slate-500 dark:text-slate-455 mt-1 leading-relaxed">{notif.message}</p>
                  <p className="text-[9px] text-slate-400 dark:text-slate-500 mt-1">Sent by: {notif.senderName}</p>
                </div>
              ))
            )}
          </div>
        ...
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm text-left space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
              <User className="w-4.5 h-4.5 text-primary-500" /> Academic & Activity Timeline
            </h3>
            ...
            {!stats.timeline || stats.timeline.length === 0 ? (
              ...
              stats.timeline.map((event: any, idx: number) => (
                <div key={idx} className="relative">
                  {/* Timeline dot */}
                  <div className={`absolute -left-[31px] top-1 w-2.5 h-2.5 rounded-full border border-white dark:border-slate-900 ${
