...
export const AcademicRecords: React.FC = () => {
  ...
  const marksData = response?.data || { marks: [], summary: [] };
  ...
  return (
    <div className="space-y-6 text-left">
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">Academic Records</h2>
          <p className="text-xs text-slate-400">View your semester-wise grades, marks, and SGPA/CGPA history details.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm justify-between items-start md:items-center">
        ...
        {latestSummary && (
          <div className="flex items-center gap-6 text-xs bg-slate-50 dark:bg-slate-800/30 p-3 rounded-xl border border-slate-100 dark:border-slate-850">
            <div>
              <p className="text-[10px] text-slate-400 uppercase font-semibold">Latest SGPA</p>
              <p className="font-bold text-slate-800 dark:text-slate-200 text-sm mt-0.5">{latestSummary.sgpa}</p>
            </div>
            <div className="border-l border-slate-200 dark:border-slate-700 h-6 shrink-0" />
            <div>
              <p className="text-[10px] text-slate-400 uppercase font-semibold">Cumulative CGPA</p>
              <p className="font-bold text-slate-800 dark:text-slate-200 text-sm mt-0.5">{latestSummary.cgpa}</p>
            </div>
            <div className="border-l border-slate-200 dark:border-slate-700 h-6 shrink-0" />
            <div>
              <p className="text-[10px] text-slate-400 uppercase font-semibold">Total Credits</p>
              <p className="font-bold text-slate-800 dark:text-slate-200 text-sm mt-0.5">{latestSummary.totalCredits}</p>
            </div>
          </div>
        )}
      </div>
